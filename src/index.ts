...
type RouterWaitState =
  | { kind: "api-wait"; target: string }
  | { kind: "streaming"; target: string }
  | { kind: "retry-backoff"; delayMs: number; pass: number; maxRetries: number };

let routesConfig: RoutesConfig = { routes: {} };
let routesCacheMtime: number | undefined;
let routesCachePath: string | undefined;
let modelsJsonCache: { mtimeMs: number; providers: Record<string, ModelsJsonProvider> } | undefined;
...
let onNotify: ((message: string, level: "info" | "warning" | "error") => void) | undefined;
let selectionClock = 0;

const cooldowns = new Map<string, number>();
const cooldownReasons = new Map<string, { class: FailureClass; error: string; at: number }>();
...
function loadRoutes(): void {
  ...
  for (const p of paths) {
    ...
    try {
      ...
      if (routesCachePath === p && routesCacheMtime === mtime) return;
...
function stateFor(target: RouteTarget): TargetRuntimeState {
...
function isOnCooldown(target: RouteTarget): boolean {
  const until = cooldowns.get(targetKey(target));
  return !!until && until > Date.now();
}

function getAvailableTargets(routeId: string): RouteTarget[] {
...
export function parseSseErrorJson(message: string): Record<string, unknown> | undefined {
  ...
  if (trimmed.startsWith("{")) {
    try {
      ...
      if (inner && typeof inner.message === "string") {
        ...
        if (code && code !== "upstream_error") return inner;
      }
      return inner;
    } catch {}
  }
...
function logEvent(ev: Omit<AutoRouterLogEvent, "ts">): void {
  if (process.env.MODEL_AUTO_ROUTER_LOG === "off") return;
  try {
...
export function readLogTail(n: number): AutoRouterLogEvent[] {
  try {
    const lines = readFileSync(getLogPath(), "utf-8").split("\n").filter(Boolean);
    const out: AutoRouterLogEvent[] = [];
    for (const line of lines.slice(-n)) {
      try {
        out.push(JSON.parse(line) as AutoRouterLogEvent);
      } catch {}
    }
    return out;
  } catch {
...
function getDb(): DatabaseHandle | undefined {
  if (db) return db;
  try {
    if (!DatabaseSync) {
    ...
    for (const col of ["cls TEXT", "error TEXT", "at INTEGER"]) {
      ...
      if (!names.has(name)) db.exec(`ALTER TABLE cooldowns ADD COLUMN ${col}`);
    }
    return db;
  } catch {
    return undefined;
...
function loadCooldowns(now = Date.now()): void {
  const handle = getDb();
  if (!handle) return;
  try {
    const rows = handle.prepare("SELECT key, until, cls, error, at FROM cooldowns WHERE until > ?").all(now) as Array<{ key: string; until: number; cls?: string; error?: string; at?: number }>;
    for (const row of rows) {
      cooldowns.set(row.key, row.until);
      if (row.cls || row.error) {
        cooldownReasons.set(row.key, {
          class: (row.cls as FailureClass) ?? "transient",
          error: row.error ?? "",
          at: row.at ?? now,
        });
      }
...
function getModelsJsonProvider(providerId: string): ModelsJsonProvider | undefined {
...
export function resolveConfigValue(raw: string | undefined): string | undefined {
  ...
  if (direct) return process.env[direct[1]];
  if (raw.includes("$")) {
...
function resolveApiKey(target: RouteTarget): string | undefined {
...
function routeModelMeta(route: RouteDefinition): RouteModelMeta {
  const metas = (route.targets ?? []).map(buildModel);
  if (metas.length === 0) return { contextWindow: 400_000, maxTokens: 16_384, reasoning: false, input: ["text"] };
  return {
...
function pushError(stream: AssistantMessageEventStream, model: Model<Api>, message: string, reason: "error" | "aborted" = "error"): void {
  stream.push({
    type: "error",
    reason,
    error: {
...
function streamWithAutoRouter(deps: Deps, model: Model<Api>, context: Context, options?: SimpleStreamOptions): AssistantMessageEventStream {
  ...
  void (async () => {
    ...
    for (const { target, error } of transientFailures.values()) putOnCooldown(target, "transient", error);
...
function getHiddenProviderIds(): Set<string> {
  const shown = new Set(routesConfig.show ?? []);
  const ids = new Set<string>();
  for (const id of routesConfig.hide ?? []) {
    if (id && id !== PROVIDER_ID && !shown.has(id)) ids.add(id);
  }
  for (const route of Object.values(routesConfig.routes)) {
    for (const target of route.targets ?? []) {
      if (target.provider && target.provider !== PROVIDER_ID && !shown.has(target.provider)) ids.add(target.provider);
    }
  }
  return ids;
}

function createStatusLine(routeId?: string): string {
  if (!routeId || !routesConfig.routes[routeId]) return "auto-router: idle";
  const now = Date.now();
  const route = routesConfig.routes[routeId];
  ...
  for (const [id, route] of Object.entries(routesConfig.routes)) {
    ...
    for (const target of route.targets ?? []) {
      ...
      if (until && until > Date.now()) {
        ...
        if (reason) status += `, ${reason.class}: ${reason.error}`;
      }
...
function isSlashArgumentContextText(textBeforeCursor: string): boolean {
  const text = textBeforeCursor.trimStart();
  return text.startsWith("/") && text.includes(" ");
}

export function createAutoRouterAutocompleteWrapper(current: any): any {
  return {
    ...
    shouldTriggerFileCompletion(lines: string[], cursorLine: number, cursorCol: number) {
      const before = (lines[cursorLine] ?? "").slice(0, cursorCol);
      if (isSlashArgumentContextText(before)) return true;
      return current.shouldTriggerFileCompletion?.(lines, cursorLine, cursorCol) ?? true;
    },
  };
}

export function createModelAutoRouterExtension(deps: Partial<Deps> = {}) {
  const resolvedDeps = { ...defaultDeps, ...deps };
  const hiddenProviders = new Set<string>();

  return function modelAutoRouter(pi: ExtensionAPI): void {
    loadRoutes();
    loadCooldowns();
    ...
    function hideTargetProviders(): void {
      loadRoutes();
      const wanted = getHiddenProviderIds();
      for (const id of wanted) {
        if (hiddenProviders.has(id)) continue;
        try {
    ...
    function registerArgumentAutocompleteFix(ctx: any): void {
      if (typeof ctx.ui?.addAutocompleteProvider !== "function") return;
      ctx.ui.addAutocompleteProvider((current: any) => createAutoRouterAutocompleteWrapper(current));
    }

    async function commandHandler(args: string, ctx: any): Promise<void> {
      ...
      if (sub === "debug") {
        ...
        const sample = available.slice(0, 15).map((entry: any) => `${entry.provider}/${entry.id} api=${entry.api}`).join("\n");
