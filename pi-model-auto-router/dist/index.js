...
function addWaitDuration(state, elapsedMs) {
    ...
    if (state.kind === "retry-backoff" || state.kind === "target-retry")
...
export function perTargetRetries() {
...
export function perTargetBackoffDelay(attempt, retry) {
...
export function retryEmptyResponses() {
...
export function isEffectivelyEmptyResponse(message) {
        ...
        while (true) {
            ...
            while (selected) {
                ...
                const abortRequest = (error) => {
                    ...
                    setRouterWaitState(undefined, routeId);
                    try {
                        pushError(outer, model, error, "aborted");
                    }
                    catch { }
                    finishRunSummary(routeId, "aborted", failovers);
                    wakeLoop?.();
                ...
                try {
                    ...
                    delete streamOptions.maxRetries;
                    delete streamOptions.maxRetryDelayMs;
                    ...
                    while (true) {
                        ...
                        if (event.type === "error" && !committed) {
                            ...
                            if (nextTarget)
                                onNotify?.(`[auto-router] ${key} failed (${cls}), trying ${targetKey(nextTarget)}`, "info");
                            break;
                        }
                        if (!committed) {
                            if (event.type === "done") {
                ...
                if (stalledByWatchdog) {
                    ...
                    if (nextTarget)
                        onNotify?.(`[auto-router] ${key} stalled (no events for ${formatDuration(stallTimeoutMs())}), trying ${targetKey(nextTarget)}`, "info");
                }
